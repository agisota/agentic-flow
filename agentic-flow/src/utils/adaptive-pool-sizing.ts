...
export interface PoolConfig {
  ...
  maxSize?: number;
  initialSize?: number;
  adjustInterval?: number; // How often to adjust (ms)
  ...
  scaleStep?: number; // How much to scale by
}
...
export class AdaptivePoolSizingManager {
   ...
   * Record pool usage
   */
  recordUsage(active: number, total?: number): void {
   ...
   */
  recordTraffic(pattern: TrafficPattern): void {
  ...
  private calculateAverageUtilization(): number {
    if (this.utilizationSamples.length === 0) return 0;
   ...
   */
  private analyzeTrafficTrend(): 'increasing' | 'stable' | 'decreasing' {
    if (this.trafficHistory.length < 10) return 'stable';
   ...
   * Update statistics
   */
  private updateStats(utilization: number): void {
   ...
   */
  private startAdjustmentMonitoring(): void {
 ...
 */
export class AdaptiveConnectionPool<T> {
  private pool: T[] = [];
  private inUse: Set<T> = new Set();
  private sizingManager: AdaptivePoolSizingManager;
  private createFn: () => T;
...
export class AdaptiveBufferPool {
  private pool: AdaptiveConnectionPool<Buffer>;
  private bufferSize: number;

  constructor(config: PoolConfig, bufferSize: number = 64 * 1024) {
    this.bufferSize = bufferSize;

    this.pool = new AdaptiveConnectionPool<Buffer>(
      config,
      () => Buffer.allocUnsafe(this.bufferSize),
      (_item: Buffer) => { /* Buffers are garbage collected */ }
    );
  }
  ...
  getStats(): PoolStats & { poolSize: number; inUseCount: number } {
 ...
 */
export function calculatePoolSizingSavings(
  oldAvgSize: number,
  newAvgSize: number,
  peakSize: number
): {
